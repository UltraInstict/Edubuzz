...
export const GET: APIRoute = async ({ params }) => {
  ...
  const xml = `<?xml version="1.0" encoding="UTF-8"?><source><publisher>Edubuzz</publisher>${jobs.map((j:any)=>`<job><title>${cdata(j.title)}</title><date>${cdata(j.created)}</date><referencenumber>${cdata(j.id)}</referencenumber><url>${cdata(`https://edubuzz.co.za/job/${j.slug}`)}</url><company>${cdata(j.company)}</company><city>${cdata(j.city)}</city><state>${cdata(j.province)}</state><country>${cdata('South Africa')}</country><description>${cdata(j.description)}</description><category>${cdata(j.category)}</category><jobtype>${cdata(j.job_type)}</jobtype><expirationdate>${cdata(j.expires)}</expirationdate></job>`).join('')}</source>`;
