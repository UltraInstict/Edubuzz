...
// ─── Combined onRecordCreate (no filter — handles multiple collections) ──
onRecordCreate((e) => {
...
// ─── Single onRecordAfterUpdateSuccess ──────────────────────────────────
onRecordAfterUpdateSuccess((e) => {
  const name = e.record.collection().name;

  // Auto-approve pending jobs
  if (name === 'pending_jobs' && e.record.get('status') === 'approved') {
    try {
  ...
  if (name === 'jobs' && !_saving) {
    const expires = e.record.get('expires');
    if (expires && new Date(expires).getTime() <= Date.now() && e.record.get('active') !== false) {
      _saving = true;
      try {
        ...
        $app.logger().error('Failed to deactivate expired job', 'error', err);
      }
      _saving = false;
    }
  }
  ...
  // Audit log
  auditLog('record_updated', { collection: name, id: e.record.id });
});
    ...
    for (const job of expiringJobs) {
      ...
      try {
        ...
        $app.newMailClient().send({
          ...
          html: `<p>Hi there,</p>
<p>Your job listing for <strong>${title}</strong> at ${company} will expire on <strong>${new Date(job.get('expires')).toLocaleDateString('en-ZA')}</strong>.</p>
...
<p style="margin-top:12px"><a href="https://edubuzz.co.za/job/${slug}">View listing</a></p>
<p style="color:#888;font-size:12px">Edubuzz.co.za \u2014 Find jobs in South Africa</p>`,
        });
